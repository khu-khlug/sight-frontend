...
export default function BookManageContainer() {
  ...
  return (
    <>
      <Container>
        <Heading size="xl" mb={4}>도서 관리</Heading>
      ...
      {activeMode === null && (
        <Container>
          ...
          {borrowsError && <Callout type="error">{extractErrorMessage(borrowsError)}</Callout>}
      ...
      {activeMode === "delete" && (
        <Container>
          ...
          {bookListError && <Callout type="error">{extractErrorMessage(bookListError)}</Callout>}
            ...
            <Box overflowX="auto">
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #e2e8f0", textAlign: "left" }}>
                    <th style={{ padding: "8px" }}>도서명</th>
                    <th style={{ padding: "8px", textAlign: "center" }}>총 권수</th>
                    <th style={{ padding: "8px", textAlign: "center" }}>대출 중</th>
                      ...
                      <tr key={book.bookId} style={{ borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <td style={{ padding: "8px" }}>
                          <Button
                            ...
                            disabled={!canDelete}
                            onClick={() => canDelete && setConfirmBook({ id: book.bookId, title: book.title })}
                          >
                            삭제
                          </Button>
                        </td>
        ...
        <Portal>
          ...
          <Dialog.Positioner>
            <Dialog.Content>
              ...
              <Dialog.Body>
                <Text>
                  <Text as="span" fontWeight="bold">"{confirmBook?.title}"</Text>을(를) 삭제할까요?
                </Text>
                <Text fontSize="sm" color="gray.500" mt={1}>이 작업은 되돌릴 수 없습니다.</Text>
              </Dialog.Body>
              <Dialog.Footer>
